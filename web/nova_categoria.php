<html>
    <head>
        <meta charset="UTF-8">
        <link rel="stylesheet" type="text/css" href="style.css">
    </head>
    <body>
        <h3>Adicionar nova categoria:</h3>
        <form action="insert_cat.php" method="post">
            <p><input type="hidden" name="nome_cat" value="<?=$_REQUEST['nome_cat']?>"/></p>
            <p><input type="hidden" name="is_supercat" value="<?=$_REQUEST['is_supercat']?>"/></p>
            <p>Nome da categoria: <input type="text" name="nome_cat"/></p>
            <p>Tipo da categoria:</p>
            <p><input type="radio" name="is_supercat" value="no" checked="checked"> Simples <br>
               <input type="radio" name="is_supercat" value="yes"> Supercategoria </p>
            <p><input type="submit" value="Adicionar"/> <a href="supermercado.php">Cancelar</a></p>
        </form>
    </body>
</html>